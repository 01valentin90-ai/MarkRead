## Hints:

---
**Ikonki użyte w plikach MD:** to nic innego jak unicode

- Skróty klawiszowe
   - MacOs🍎 -> ctrl + cmd + space
   - Windows 🪟 -> win + .lub win + ;
***

# 📘 ProjectX - Dokumentacja Techniczna 
***
**Wersja:** 2.3.7
**Data ostatniej aktualizacji:** 2025-02-14
**Autor:** Zespół Frontend - Akademia Nowych Technologii
***
## 📌 Spis treści
***
1. [Opis projektu]()
2. [Specyfikacja techniczna]()
3. [Wymagania systemowe]()
4. [Instalacja]()
5. [Struktura katalogów]()
6. [API - Endpoints]()
7. [Konfiguracja środowiska]()
8. [Tabela testów]()
9. [Lista zadań do wykonywania]()
10. [Licencja]()

***
## 🔍 Opis Projektu
***
**Project** to przykładowa aplikacja frontendowa zbudowana w ramach kursu.
Główne cele projektu:
- prezentacja działania podstawowego API,
- wykorzystanie lokalnego serwera deweloperskiego,
- demonstracja struktury projektowej, 
- praca z dokumentacją w formacie **Markdown**.
>"Dokumentacja jest tak samo ważna jak kod." - autor nieznany 
***
## ⚙️ Specyfikacja techniczna 
***
**Technologie**:
- HTML5
- CSS# (BEM, zmienne CSS)
- JavaScript ES2022
- Node.js **v18.12.1**
- npm **8.19.3**

**Build tools:**
- Vite **4.5.0**
- ESLint **8.57.0**
***
## 🖥️ Wymagania systemowe
***
|**Komponent**|**Minimalne**|**Rekomendowane**|
|----|----|----|
|RAM|4 GB|8 GB|
|Procesor|2 rdzenie|4 rdzenie|
|Node.js|16.x|18.x|
|Przeglądarka|Chrome 95|Chrome 120|
***
## 🧩 Instalacja 
***
## 1. Klonowanie repozytorium
```
git clone https://example.com/projectx.git cd projectx
